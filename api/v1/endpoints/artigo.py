...
async def post_artigo(artigo: ArtigoSchema, usuario_logado: UsuarioModel = Depends(get_corrent_user),db: AsyncSession = Depends(get_session)):
...
async def put_artigo(artigo_id: int, artigo: ArtigoSchemaUp, db: AsyncSession = Depends(get_session), usuario_logado: UsuarioModel = Depends(get_corrent_user)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
...
async def delete_artigo(artigo_id: int, usuario_logado: UsuarioModel = Depends(get_corrent_user), db: AsyncSession = Depends(get_session)):
    async with db as session:
